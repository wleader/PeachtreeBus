...
namespace PeachtreeBus.DataAccessTests
{
    ...
    public class ManagementGetFailedQueueMessagesFixture : ManagementDataAccessFixtureBase
    {
        ...
        {
            var action = new Func<string, Task>(async (s) => await dataAccess.GetFailedQueueMessages(s, 1, 2));
        ...
        {
            var action = new Func<Task>(async () => await dataAccess.GetFailedQueueMessages(DefaultQueue, 1, 2));
